...
import { StyleConfig, DEFAULT_STYLE_CONFIG } from './types';
...
export async function startJob(params: StartJobParams) {
  ...
  const styleConfig = params.styleConfig ?? DEFAULT_STYLE_CONFIG;
  ...
  try {
    ...
    for (const clip of plan) {
      ...
      try {
        const outputPath = await renderClip({ sourcePath, clip, outDir, styleConfig });
